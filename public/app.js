...
function calculate() {
    ...
    // Workers - \uc870\uac74\ud45c\uc758 \uc778\ub825\uac12\uc744 \uadf8\ub304\ub85c \uac00\uc838\uc634 (override \ubc18\uc601)
    ...
    // Costs - override\uc744 \ubc18\uc601\ud55c \uc720\ud6a8\ud55c \uc870\uac74\uac12 \uc0ac\uc6a9
...
function updateUI() {
    ...
    document.getElementById('card-summary').style.display = hasArea ? 'block' : 'none';
...
function renderTabs() {
    ...
    document.getElementById('tbl-q-total').innerHTML = `
        ...
        <tr><td>연면적</td><td>${state.floorArea.toLocaleString()} ㎡</td><td>등급: ${state.results.grade}</td></tr>
        ...
        ${discountRow}
        <tr style="font-weight:bold; color:var(--primary-color)"><td>최종 합계 (연간)</td><td>₩ ${state.results.costs.yearly.toLocaleString()}</td><td>부가세 별도</td></tr>
        <tr style="font-weight:bold"><td>월 납부액</td><td>₩ ${state.results.costs.monthly.toLocaleString()}</td><td>÷12</td></tr>
    `;
    ...
    document.getElementById('tbl-q-inspection').innerHTML =
        ...
        <tr style="font-weight:700; color:var(--primary-color)">
            <td>산출 합계</td><td></td><td>₩ ${inspB.total.toLocaleString()}</td>
        </tr>
        <tr style="color:${state.results.costs.inspection - inspB.total >= 0 ? '#16a34a' : '#dc2626'}">
            <td>조정 금액</td>
            <td>목표금액 − 산출합계</td>
    ...
    document.getElementById('tbl-q-maintenance').innerHTML =
        ...
        <tr style="color:${state.results.costs.maintenance - maintB.total >= 0 ? '#16a34a' : '#dc2626'}">
            <td>조정 금액</td>
            <td>목표금액 − 산출합계</td>
    ...
    document.getElementById('tbl-q-appointment').innerHTML = `
        ...
        <tr style="font-weight:bold"><td>연간 선임 합계</td><td></td><td>₩ ${state.results.costs.appointment.toLocaleString()}</td></tr>
    `;
    ...
    ['tab2', 'tab3'].forEach(tabId => {
        ...
        btn.style.cssText = [
            ...
            'color:#6b7280', 'display:flex', 'align-items:center', 'gap:0.4rem'
        ].join(';');
        ...
        panel.innerHTML = (() => {
            const wageHtml = GRADE_ORDER.map(g => {
                ...
                return `<div style="display:flex;align-items:center;justify-content:space-between;padding:0.28rem 0;border-bottom:1px solid #f9fafb;gap:0.5rem;">
                    <span style="display:flex;align-items:center;gap:0.4rem;">
                        <span style="display:inline-flex;align-items:center;justify-content:center;width:36px;flex-shrink:0;">${a ? '<span style="background:var(--primary-color);color:white;font-size:0.62rem;font-weight:700;padding:1px 5px;border-radius:999px;">\uc801\uc6a9</span>' : ''}</span>
                        <span style="font-size:0.81rem;font-weight:${a ? '700' : '400'};color:${a ? 'var(--primary-color)' : '#374151'};white-space:nowrap;">${g} \uae30\uc220\uc790</span>
                    </span>
                    <span style="font-size:0.81rem;font-weight:${a ? '700' : '500'};color:${a ? 'var(--primary-color)' : '#6b7280'};font-variant-numeric:tabular-nums;white-space:nowrap;">₩ ${GRADE_WAGES[g].toLocaleString()}</span>
                </div>`;
            }).join('');
            ...
            const coefHtml = ADJUSTMENT_COEFFICIENTS.map(c => {
                ...
                return `<div style="background:${a ? 'var(--primary-color)' : '#f1f5f9'};color:${a ? 'white' : '#475569'};border-radius:8px;padding:0.35rem 0.65rem;text-align:center;white-space:nowrap;border:1px solid ${a ? 'var(--primary-color)' : '#e2e8f0'};">
                    <div style="font-size:0.62rem;opacity:${a ? .85 : .7};margin-bottom:1px;">${label || c.area.toLocaleString() + '\u33a1~'}</div>
                    <div style="font-size:0.88rem;font-weight:700;">${c.coef.toFixed(2)}</div>
                </div>`;
            }).join('');

            const COND_ROWS = [
                { label: '\ub4f1\uae09', fn: c => `<span style="font-weight:700;color:${GRADE_STYLES[c.grade]?.color || '#374151'}">${c.grade}</span>` },
            ...
            const thCells = QUOTATION_CONDITIONS.map(c => {
                ...
                return `<th style="padding:0.38rem 0.5rem;font-size:0.67rem;font-weight:${a ? '700' : '500'};background:${a ? 'var(--primary-color)' : '#f8fafc'};color:${a ? 'white' : '#6b7280'};border:1px solid #e5e7eb;text-align:center;white-space:nowrap;">
                    ${(COND_RANGE_LABELS[c.area] || '').replace(' \u2264 \uc5f0\uba74\uc801 <', '<br><').replace(/\u33a1/g, '\u33a1')}
                    ${a ? '<div style="font-size:0.58rem;opacity:.8;margin-top:2px;">\u25b2 \ud604\uc7ac \uc801\uc6a9</div>' : ''}
                </th>`;
            }).join('');
            ...
            const bodyRows = COND_ROWS.map(row => {
                const tds = QUOTATION_CONDITIONS.map(c => {
                    const a = c.area === condition.area;
                    return `<td style="padding:0.36rem 0.5rem;font-size:0.79rem;text-align:center;border:1px solid #e5e7eb;background:${a ? '#eff6ff' : 'white'};font-weight:${a ? '600' : '400'};color:${a ? '#1d4ed8' : '#374151'};white-space:nowrap;">${row.fn(c)}</td>`;
                }).join('');
                return `<tr>
                    <td style="padding:0.36rem 0.65rem;font-size:0.75rem;color:#6b7280;background:#f8fafc;border:1px solid #e5e7eb;white-space:nowrap;font-weight:500;">${row.label}</td>
                    ${tds}
                </tr>`;
            }).join('');

            return `
            <div style="display:grid;grid-template-columns:auto 1fr;gap:0.85rem;margin-bottom:0.85rem;">
                <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:0.7rem 1rem;min-width:190px;">
                    <div style="font-size:0.68rem;font-weight:700;color:var(--primary-color);letter-spacing:.04em;padding-bottom:.3rem;border-bottom:1px solid #f0f0f0;margin-bottom:.4rem;">\ud83d\udcb0 \ub4f1\uae09\ubcc4 \ub178\uc784\ub2e8\uac00 (\uc6d0/\uc778)</div>
                    ${wageHtml}
                </div>
                <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:0.7rem 1rem;">
                    <div style="font-size:0.68rem;font-weight:700;color:var(--primary-color);letter-spacing:.04em;padding-bottom:.3rem;border-bottom:1px solid #f0f0f0;margin-bottom:.5rem;">\ud83d\udd22 \uc5f0\uba74\uc801 \uc870\uc815\uacc4\uc218</div>
                    <div style="display:flex;gap:.4rem;flex-wrap:wrap;">${coefHtml}</div>
                </div>
            </div>
            <div style="background:white;border:1px solid #e5e7eb;border-radius:8px;padding:0.7rem 1rem;overflow-x:auto;">
                <div style="font-size:0.68rem;font-weight:700;color:var(--primary-color);letter-spacing:.04em;padding-bottom:.3rem;border-bottom:1px solid #f0f0f0;margin-bottom:.55rem;">\ud83d\udcca \uacac\uc801 \uc870\uac74\ud45c (\uc804\uccb4 \uad6c\uac04)</div>
                <table style="width:100%;border-collapse:collapse;min-width:560px;">
                    <thead><tr>
                        <th style="padding:0.38rem 0.65rem;font-size:0.68rem;font-weight:600;background:#f8fafc;color:#6b7280;border:1px solid #e5e7eb;text-align:left;white-space:nowrap;">\ud56d\ubaa9</th>
                        ${thCells}
                    </tr></thead>
...
async function fetchBuildingInfo() {
    ...
    try {
        ...
        statusEl.innerHTML = '✅ 건축물대장 조회 성공! "✅ 이 값으로 적용" 버튼으로 값을 입력하세요.';
    } catch (err) {
...
function initKakaoSearch() {
    window.wkCommon.initKakaoPostcode('kakao-embed-container', (roadAddr, buildingName) => {
        ...
        document.getElementById('card-address-search').style.display = 'none';
        document.getElementById('card-building-info').style.display = 'block';
        document.getElementById('card-bottom-actions').style.display = 'flex';
...
document.getElementById('floor-area').addEventListener('input', (e) => {
    ...
    if (state.floorArea > 0) {
        const buildingCard = document.getElementById('card-building-info');
...
// Reset: show address search again
document.getElementById('btn-reset-addr').addEventListener('click', () => {
    ...
    // Card visibility reset
    document.getElementById('card-address-search').style.display = 'block';
    document.getElementById('card-building-info').style.display = 'none';
    document.getElementById('card-summary').style.display = 'none';
    document.getElementById('card-detail').style.display = 'none';
    document.getElementById('card-bottom-actions').style.display = 'none';
