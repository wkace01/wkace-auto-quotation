...
### 4. 반응형 모바일 지원
- Media Query(`@media (max-width: 600px)`)를 활용한 모바일 뷰어 최적화 (뷰포트 지원)
- 현장 미팅 시 스마트폰 등 소형 화면에서도 입력 서식을 편하게 검토하고 즉석 PDF 생성 가능
...
│   ├── app.js                  # 뷰 조작 로직, 통신 로직, 계산 알고리즘, PDF 변환 요청
│   ├── style.css               # 데스크톱 및 모바일 반응형(Grid, Flex) 요소 스타일링
│   └── data.js                 # 엑셀 단가표, 조정계수, 등급별 기초 데이터 상수 모음 (존재 시)
│
